...
%значения
  ...
  kol_ot3 por_n_fila uu1 dl kol_skol probna uu2 dlin_naz_f puti uuu dosia real_otsch...    
...
if B(46)==1
   ...
   for p_chan=1:length(mas_skl_sr(1,:))
      ...
      rty=find(otk_ot_sr(:,p_chan)>sr_kv_otkl(p_chan)*F(5));%нахождение индексов массива которы входят в 95%-й интервал
      if isempty(rty)~=1
   ...
   %disp(ocenki)
   if kol_f_zam>1
      [znach,index]= sort(otk_ot_sr);% сортировка по возрастанию (в массиве idex- индексы отсортир значений)
      [znach2,index2]= sort(sr_otkl_ot_sr);%
      for dva=od% цикл (в первый раз без удаления знач для файлов с большим откл от средн)
         %       второй раз с удалением  
         if dva==2 %для второго раза в массиве index обнуляем те индексы для которых значени превысили допуст откл от сренего
            %disp (ocenki)
            for por_n1=1:kol_f_zam
               for por_n_fila1=1:kol_f_zam
                  ud=find(index(por_n1,:)==ocenki(por_n_fila1,2));
                  
                  index(por_n1,ud)=0;
               end
               %ud2=find(index2(por_n1)==ocenki(por_n1,2))
               %index2(ud2)=[]
            end
         ...
         %break;
         for por_n=1:kol_f_zam % цикл для добавления строк в анализ диапазон
            
            for por_n_fila=1:kol_f_zam % цикл по строкам
               %disp(index)
               k=find(index(1:por_n,:)==por_n_fila); %находит индексы для каждого файла (всего индексов=кол-ву выбр каналов)
               ocenki(por_n_fila,1)=length(k);       %кол-во индексов для кажд файла
            end
            max_kol_sluch=max(ocenki(:,1)); %находится максимальное кол-во случаев соответствия тому или иному файлу 
            if max_kol_sluch~=0
               kkk=find(ocenki(:,1)==max_kol_sluch);% найдем номер файла соотв максим кол-ву случаев
               kol_povt=length(kkk);
               if kol_povt<2
                  break;
               end
      ...
      %disp(nom_kan_vub)
      if kkk(1)~=nom_kan_vub
         msgbox(['Ни один файл не входит в заданный интервал.Рекомендуется-',...
               getfield(f_zam(nom_kan_vub).name)],'Сообщение')
      else
         msgbox(['Рекомендуется-',getfield(f_zam(nom_kan_vub).name)],'Сообщение')
   ...
   if kol_f_zam>1
      nazvan=getfield(f_zam(nom_kan_vub).name);
   else
