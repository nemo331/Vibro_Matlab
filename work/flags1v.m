...
uic(1)=uicontrol('Style','Popup','String',...
   tip_f,'Callback','uiresume','Position',[20/kof 445/kof 60/kof 20/kof],...
   'Value',A(1)+1);     
uicontrol('Style','text','String','Тип файла',...
          'Position',[80/kof 445/kof 220/kof 20/kof]);
...
case 2
   ...
   if (A(1)==0|A(2)==1|(strcmp(name_fl_zam(1,1),'пусто'))==1 ...
      |(strcmp(name_fl_pas(1,1),'пусто'))==1) 
   ...
   if (strcmp(name_fl_zam(1,1),'пусто'))==1
      set(ok,'Enable','off');
   else 
      set(ok,'Enable','on');
   end
...
switch gobsch
  case 2
   ...
   case 3
      ...
      exst_f = dir(strcat(puti{5,2},'Viewpasp.exe'));
      if isempty(exst_f) == 0
         %Запуск функции FOXPRO для просмотра паспортов 
         passport_vfp(kot,pas,name_fl_zam,puti);
      else
